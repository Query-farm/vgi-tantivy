#!/usr/bin/env bash
# Build the browser (`worker:`) build of the tantivy VGI worker:
# tantivy-wasm compiled to wasm32-unknown-emscripten, linked by emcc into a
# MODULARIZE'd Web Worker module.
#
...
#     recompiles compiler_builtins with atomics. Also `rustup component add
#     rust-src` for nightly and the wasm32-unknown-emscripten target.
set -euo pipefail
cd "$(dirname "$0")/.."
...
LIB="target/$TARGET/release/libtantivy_wasm.a"
...
# which this worker overflows — an overflow surfaces as a bare "memory access out
# of bounds" with no console output, not a recognizable stack error. The serve
# threads are the ones that actually run work, so the pthread stack matters most.
