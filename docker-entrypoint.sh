...
#
# Dispatch the single vgi-tantivy image into one of its transports:
...
#
# The worker is stateless (per-call in-RAM index, corpora passed inline), so
...
case "${1:-http}" in
  http)
    ...
    exec tantivy-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec tantivy-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec tantivy-worker "$@"
