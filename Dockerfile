...
#
# Single image that serves every network transport of the `tantivy` VGI worker:
...
#
# This worker is STATELESS: every full-text index is built in a RAM directory
# per call and dropped immediately, and corpora arrive inline as constant JSON.
# There is no /data volume, no persisted index, and no `farm.query.vgi.volumes`
...
# runtime ships. rust:1-bookworm tracks the latest stable 1.x toolchain, which
# satisfies the workspace `rust-version = 1.90`.
...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cargo build --release --locked --bin tantivy-worker \
    && cp target/release/tantivy-worker /tantivy-worker
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-tantivy
...
LABEL org.opencontainers.image.title="vgi-tantivy" \
      org.opencontainers.image.description="Full-text search (BM25 ranking), tokenization, and Snowball stemming as a VGI worker for DuckDB/SQL (stdio + HTTP + TCP)" \
...
ENV PORT=8000 \
    ...
    VGI_TANTIVY_GIT_COMMIT=${GIT_COMMIT}
...
COPY --from=build --chmod=0755 /tantivy-worker /usr/local/bin/tantivy-worker
