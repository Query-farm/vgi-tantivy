//! The `tantivy` VGI worker (library).
//!
...
//! the `tantivy-wasm` crate (the browser build, which serves the same `Worker`
...
/// The catalog name DuckDB sees in `ATTACH 'tantivy' (TYPE vgi, …)`. Defaults to
/// `tantivy`, but honors an explicit override so a test harness can rename it.
